...
async function boot() {
  ...
  if (status === true) {
    // Aunque tengamos handle y permiso, verificar que la carpeta aún exista físicamente
    const exists = await storage.folderExists();
    if (!exists) {
      showFolderGate('deleted');
      return;
    }
    afterDataReady();
...
// mode: 'new' | 'needs-permission' | 'deleted'
function showFolderGate(mode) {
  // Bloquea toda la interfaz con un overlay hasta que se resuelva
  ...
  if (mode === 'new') {
    // Pantalla de bienvenida amigable con pasos visuales
    overlay.innerHTML = `
      <div style="max-width:480px;width:90%;text-align:center;padding:20px;">

        <!-- Logo / ícono grande -->
        <div style="width:72px;height:72px;border-radius:18px;background:linear-gradient(160deg,#E8AA4C,#c98a2e);
             display:flex;align-items:center;justify-content:center;margin:0 auto 22px;
             font-weight:700;color:#1a1308;font-size:32px;box-shadow:0 8px 24px rgba(232,170,76,.3);">
          P
        </div>

        <h1 style="margin:0 0 8px;font-size:26px;color:var(--text);font-weight:700;">
          Proyector de Servicio
        </h1>
        <p style="color:var(--text-muted);font-size:14px;margin:0 0 32px;line-height:1.6;">
          Tu asistente para controlar lo que se ve en el proyector durante el culto.<br>
          Solo necesitas hacer una cosa antes de empezar:
        </p>
        ...
        <div style="background:var(--panel);border:1px solid var(--border);border-radius:12px;
          ...
          <div style="display:flex;align-items:flex-start;gap:16px;">
            <div style="width:36px;height:36px;border-radius:50%;background:var(--amber);
                 display:flex;align-items:center;justify-content:center;flex-shrink:0;
                 font-weight:700;color:#1a1308;font-size:16px;">1</div>
            <div>
              <div style="font-weight:600;color:var(--text);font-size:14px;margin-bottom:6px;">
                Elige una carpeta para guardar tus datos
              </div>
              <div style="color:var(--text-muted);font-size:13px;line-height:1.6;">
                Aquí se guardarán tus canciones, anuncios, citas y listas de servicio.
                Puedes crear una carpeta nueva o usar una que ya tengas —
                <strong style="color:var(--text);">solo asegúrate de no borrarla después.</strong>
              </div>
            </div>
          </div>
        </div>

        <button class="btn btn-primary" id="btnChooseFolder"
          style="width:100%;justify-content:center;padding:14px;font-size:15px;border-radius:10px;">
          📁 &nbsp; Elegir mi carpeta de datos
        </button>

        <p style="margin-top:16px;font-size:11.5px;color:var(--text-faint);line-height:1.6;">
          Esta configuración solo se hace una vez. Después la app la recuerda automáticamente.
        </p>
      </div>`;
    ...
    overlay.innerHTML = `
      <div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;
           padding:38px 40px;max-width:400px;width:90%;text-align:center;">
        <div style="font-size:44px;margin-bottom:16px;">🔑</div>
        <h2 style="margin:0 0 10px;font-size:18px;color:var(--text);">Un momento antes de continuar</h2>
        <p style="color:var(--text-muted);font-size:13.5px;line-height:1.6;margin:0 0 26px;">
          Por seguridad, Chrome pide confirmar el acceso a tu carpeta de datos cada vez que abres la app.
          Solo toma un segundo.
        </p>
        <button class="btn btn-primary" id="btnChooseFolder"
          style="width:100%;justify-content:center;padding:12px;">
          Conceder acceso a mi carpeta
        </button>
      </div>`;
    ...
    overlay.innerHTML = `
      <div style="background:var(--panel);border:1px solid var(--border);border-radius:14px;
           padding:38px 40px;max-width:420px;width:90%;text-align:center;">
        <div style="font-size:44px;margin-bottom:16px;">📂</div>
        <h2 style="margin:0 0 10px;font-size:18px;color:var(--text);">No encontramos tu carpeta de datos</h2>
        <p style="color:var(--text-muted);font-size:13.5px;line-height:1.6;margin:0 0 10px;">
          Parece que la carpeta donde guardabas tus datos fue borrada o movida de lugar.
        </p>
        <p style="color:var(--text-muted);font-size:13.5px;line-height:1.6;margin:0 0 26px;">
          Si la moviste, búscala y selecciónala. Si ya no existe, crea una carpeta nueva —
          tus listas y canciones anteriores se perdieron, pero puedes volver a agregarlas.
        </p>
        <button class="btn btn-primary" id="btnChooseFolder"
          style="width:100%;justify-content:center;padding:12px;">
          📁 &nbsp; Elegir o crear carpeta de datos
        </button>
        <p style="margin-top:14px;font-size:11.5px;color:var(--text-faint);">
          Esta pantalla no desaparece hasta que elijas una carpeta válida.
        </p>
      </div>`;
  }
  ...
  document.getElementById('btnChooseFolder').onclick = async () => {
    try {
      ...
      if (mode === 'needs-permission') {
        success = await storage.requestPermission();
        if (!success) { return; }
      } else {
      ...
      // usuario canceló el selector — el overlay permanece visible
    }
  };
}
